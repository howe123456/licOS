...
> 基于 Arch Linux 的发行版，配备现代化 TUI 安装器、Plymouth 闪电动画开机画面和 XFCE 桌面环境 — v2.1

licOS 是一个基于 Arch Linux 的发行版，专注于简洁和现代桌面计算。它配备了基于 curses 的终端界面安装器，支持 12 种语言、LUKS 加密、灵活分区、在开机和安装器中动态劈下的闪电徽标，以及通过交互式 TUI 启动器展示 XFCE 暗色桌面主题（licOS-dark）的功能。

## 特性

### v2.1 更新

- **CJK 字体修复** — wqy-zenhei 改为通过 pacman 安装（而非直接放置文件），确保 fontconfig 缓存在 ISO 构建时已生成，XFCE 终端开机立即可用，无需等待 fc-cache
- **licos-cjk.service** — 新增专用服务，在 `graphical.target` 之前运行 `fc-cache -f`，保证字体加载
...
- **licos-launcher** — TUI 主菜单启动器（C 二进制，约 15K），含桌面展示入口
- **licos-about** — 交互式桌面展示（TUI，C 二进制，约 19K），展示 licOS 特色功能
- **licos-fetch** — 系统信息 CLI 工具，带闪电 ASCII 艺术（C 二进制，约 15K）
- **licos-welcome** — 交互式欢迎仪表盘（TUI，C 二进制，约 15K）
- **licos-setup** — 首次启动配置服务（C 二进制，约 15K），自动应用系统覆盖配置
...
1. 从 [Releases](https://github.com/howe123456/licOS/releases/tag/v2.1) 下载最新 ISO
2. 写入 U 盘：
   ```bash
...
│   │   │   │   ├── backgrounds/ # 闪电壁纸
│   │   │   └── overrides/   # 首次启动覆盖配置（zshrc、XFCE 面板）
│   │   │       ├── root.zshrc
│   │   │       ├── skel.zshrc
│   │   │       ├── xfce4-panel.xml
│   │   │       └── xfce4-xsettings.xml
