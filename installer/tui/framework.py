...
class LightningLogo:
    ...
    def draw(self, win, y, x):
        frame, color_name = LIGHTNING_FRAMES[self.frame_idx]
        attr = color(color_name) | curses.A_BOLD
...
class Application:
    ...
    def run(self):
        ...
        while self.running:
            ...
            logo_x = max_x - self.logo.width - 2
            if logo_x > sidebar_w + 2:
                self.logo.draw(self.stdscr, 0, logo_x)
                self.logo.update()
